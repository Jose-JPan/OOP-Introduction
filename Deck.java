...
public class Deck {
    ...
    public Deck(boolean includeJokers) {
        if (includeJokers)
            deck = new Card[54]; // Declare an array of type Card (from 0 t0 53 = 54 cards)
        else
            deck = new Card[52];
        int cardCt = 0; // How many cards have been created so far.
        for ( int suit = 0; suit <= 3; suit++ ) {
            for ( int value = 1; value <= 13; value++ ) {
                deck[cardCt] = new Card(value,suit); // Construct a new object and store a 
                                                     // reference to it in the array
                
                // int cardValue = deck[cardCt].getValue();
                // int cardSuit = deck[cardCt].getSuit();
